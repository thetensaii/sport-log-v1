...
export const SearchPlayers = () => {
  const [heightFilterValues, setHeightFilterValues] =
    useState<number[]>(HEIGHT_DEFAULT);
        ...
        <Grid w="full" templateColumns="repeat(2, 1fr)" gap={16}>
          <GridItem>
            <Box>
              <Text>Taille</Text>
              <Text>
                {`${heightFilterValues[0]} cm`} -{" "}
                {`${heightFilterValues[1]} cm`}
              </Text>
              <RangeSlider
                aria-label={["min", "max"]}
                min={HEIGHT_DEFAULT[0]}
                max={HEIGHT_DEFAULT[1]}
                defaultValue={HEIGHT_DEFAULT}
                onChange={updateHeightFilter}
              >
                <RangeSliderTrack>
                  <RangeSliderFilledTrack />
                </RangeSliderTrack>
          ...
          <GridItem>
            <Box>
              <Text>Age</Text>
              <Text>
                {ageFilterValues[0]} - {ageFilterValues[1]}
              </Text>
              <RangeSlider
                aria-label={["min", "max"]}
                min={AGE_DEFAULT[0]}
